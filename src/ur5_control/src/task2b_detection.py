...
class VisionAndTFPublisher(Node):
    def __init__(self):
        ...
        # --- 1. SET DEFAULTS (So we don't have to wait) ---
        # Restoring the specific values from your original code to use as startup defaults
        ...
        # Construct default Camera Matrix
        self.camera_matrix = np.array([
            [self.focalX, 0, self.centerCamX], 
            [0, self.focalY, self.centerCamY], 
            [0, 0, 1]
        ])
        self.dist_coeffs = np.zeros((5, 1)) # Default to zero distortion
        ...
        # 1. Camera Info (Background Cross-check)
        ...
        self.get_logger().info(f"Vision Node Started. Using default parameters initially.")

    ## --- CALLBACKS ---
    
    def camera_info_callback(self, msg):
        """Cross-checks and updates camera intrinsics if they differ from defaults."""
        current_k = np.array(msg.k).reshape((3, 3))
        current_d = np.array(msg.d)
        
        # Only log if this is the first time we are receiving it or if it changes
        if not self.camera_info_received:
            self.get_logger().info("Received Camera Info Topic. Cross-checking...")
            
            # Simple check to see if our defaults were close
            if not np.allclose(self.camera_matrix, current_k, atol=1.0):
                self.get_logger().warn(
                    f"Parameters updated from topic! \n"
                    f"Default fx: {self.focalX} -> Topic fx: {current_k[0,0]}\n"
                    f"Default cx: {self.centerCamX} -> Topic cx: {current_k[0,2]}"
                )
            else:
                self.get_logger().info("Topic parameters match defaults.")

            # Update to the 'truth' from the topic
            self.camera_matrix = current_k
            self.dist_coeffs = current_d
    ...
    def detect_aruco_markers(self, image):
        ...
        try:
            ...
            # Uses whatever is currently in self.camera_matrix (Default OR Updated)
    ...
    def compute_3d_position(self, pixel_x, pixel_y, depth):
        z_depth = float(depth)
        # Uses whatever is currently set (Default OR Updated)
    ...
    def process_vision_and_tf(self):
        # NOTE: Removed blocking "not self.camera_info_received" check here
