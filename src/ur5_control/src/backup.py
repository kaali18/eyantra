...
# --- NEW: TRANSFORM OFFSETS FOR FRUIT (Meters) ---
# These are applied relative to the BASE_LINK frame.
FRUIT_TF_OFFSET_X = 0.0  # +X moves forward relative to robot base
FRUIT_TF_OFFSET_Y = 0.0  # +Y moves left relative to robot base
...
class VisionAndTFPublisher(Node):
    ...
    def match_fruit_to_tracked(self, pixel_x, pixel_y, depth):
        ...
        for fruit_id, tf_msg in self.locked_fruit_tfs.items():
            try:
                ...
                # Reverse the offset subtraction for matching logic if needed, 
                # but standard practice is to match based on the stored tf.
    ...
    def lock_fruit_tf(self, fruit_id, pos_x_cam, pos_y_cam, pos_z_cam):
        try:
            ...
            # --- APPLY OFFSETS HERE (IN BASE_LINK FRAME) ---
            tf_msg.transform.translation.x = base_pt.point.x + FRUIT_TF_OFFSET_X
            tf_msg.transform.translation.y = base_pt.point.y + FRUIT_TF_OFFSET_Y
            ...
            self.get_logger().info(f'✅ LOCKED {frame_name.upper()} with offset X:{FRUIT_TF_OFFSET_X}, Y:{FRUIT_TF_OFFSET_Y}')
        except Exception as e: 
